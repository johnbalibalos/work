...
#Reads input file and creates .out workbook and worksheets for annotation
...
#Copies original sheet from sample report
row = 0
col = 0
newrow = 0
newcol = 0
...
for col_index in range(0, 26):
	ws.write(1, col_index, "", t2_style)
	wn.write(1, col_index, "", t2_style)
				...
				for col_i in range(col, rs.ncols):
					try:
						...
						if annotation[3] == '-':
							...
							wn.write(newrow, (newcol + nsafcol), Formula('(100*' + cellnameabs(newrow,newcol+scaacol) + ')/' + cellnameabs(rs.nrows-30, newcol+scaacol)),decimal_style)
							newcol += 1
					except:
...
#print noprotein at the end of run
...
for col_index in range(col, rs.ncols):
	...
	wn.write(1, newcol, 'SC', t2_style)
	ws.write(1, newcol, 'SC', t2_style)
	wn.write(1, (newcol + (rs.ncols - col)), 'SC/AA', t2_style)
	wn.write(1, (newcol + (2*(rs.ncols - col))), '%NSAF', t2_style)
	#wn.write((rs.nrows - 29), newcol + scaacol, 'SUM SC/AA')
	#wn.write((rs.nrows - 29), newcol + nsafcol, 'SUM NSAF%')
	wn.write((rs.nrows - 30), newcol + scaacol, Formula(sumscaa),decimal_style)
	wn.write((rs.nrows - 30), newcol + nsafcol, Formula(sumnsaf),decimal_style)
	ws.write((rs.nrows - 30), 25, 'ID#', title_style)
	wn.write((rs.nrows - 30), 25, 'ID#', title_style)
	ws.write((rs.nrows - 30), newcol, Formula('COUNTIF(' + cellnameabs(2,newcol) + ':' + cellnameabs((rs.nrows-31), newcol) + ',' + '">0")'))
	wn.write((rs.nrows - 30), newcol, Formula('COUNTIF(' + cellnameabs(2,newcol) + ':' + cellnameabs((rs.nrows-31), newcol) + ',' + '">0")'))
	ws.write((rs.nrows - 29), 25, 'SC#', title_style)
	wn.write((rs.nrows - 29), 25, 'SC#', title_style)
	ws.write((rs.nrows - 29), newcol, Formula('SUM(' + cellnameabs(2,newcol) + ':' + cellnameabs((rs.nrows-31), newcol) + ')'))
	wn.write((rs.nrows - 29), newcol, Formula('SUM(' + cellnameabs(2,newcol) + ':' + cellnameabs((rs.nrows-31), newcol) + ')'))
	newcol += 1
row = 30
newrow = 2 
col = 9
...
for col_index in range(col, rs.ncols):
	for row_index in range(row, rs.nrows):
		if rs.cell(row_index, col).value != "":
			if rs.cell(row_index, col).value > 0:
				ws.write(newrow, newcol, rs.cell(row_index, col).value, sc_style)
				wn.write(newrow, newcol, rs.cell(row_index, col).value, sc_style)
				newrow += 1
			else:
				ws.write(newrow, newcol, rs.cell(row_index, col).value, sc0_style)
				wn.write(newrow, newcol, rs.cell(row_index, col).value, sc0_style)
				newrow += 1
	newrow = 2
	col += 1
	newcol += 1
